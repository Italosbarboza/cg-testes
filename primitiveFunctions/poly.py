...
def draw(buf, pol, intens):
    m = buf

    for i in range(0, pol.shape[0]-1):
        m = img.strt_line_ddaaa(m, pol[i, 0], pol[i, 1], pol[i+1, 0], pol[i+1, 1], intens)

    m = img.strt_line_ddaaa(m, pol[-1, 0], pol[-1, 1], pol[0, 0], pol[0, 1], intens)
...
def fill_circle(image, center_x, center_y, radius, start_color, end_color):
    circle_points = draw_circle(center_x, center_y, radius)
    num_points = len(circle_points)

    for i, point in enumerate(circle_points):
        progress = i / (num_points - 1)  # Calcula o progresso entre 0 e 1

        # Interpolação linear das cores
        r = int((1 - progress) * start_color[0] + progress * end_color[0])
        g = int((1 - progress) * start_color[1] + progress * end_color[1])
        b = int((1 - progress) * start_color[2] + progress * end_color[2])

        image = img.set_pixel(image, point[0], point[1], r, g, b)
...
def draw_circle(center_x, center_y, radius):
    ...
    while x <= y:
        for i in range(center_x - x, center_x + x + 1):
            points.append((i, center_y + y))
            points.append((i, center_y - y))

        for i in range(center_x - y, center_x + y + 1):
            points.append((i, center_y + x))
            points.append((i, center_y - x))
